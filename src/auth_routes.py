...
def register():
    ...
    return jsonify({
        "token": token,
        "user": {"id": user_id, "name": name, "email": email, "role": role, "is_active": 1},
    })
...
def get_preferences():
    ...
    conn = get_db_connection()
    row = conn.execute(
        "SELECT preferred_state, preferred_type, min_price, max_price FROM user_preferences WHERE user_id = ? ORDER BY id DESC LIMIT 1",
        (user["id"],),
    ).fetchone()
    conn.close()
    return jsonify({"preferences": dict_from_row(row) or {}})
...
def save_preferences():
    ...
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO user_preferences (user_id, preferred_state, preferred_type, min_price, max_price) VALUES (?, ?, ?, ?, ?)",
        (user["id"], data.get("preferred_state", ""), data.get("preferred_type", ""), data.get("min_price"), data.get("max_price")),
    )
    conn.commit()
    conn.close()
    log_event("SAVE_PREFERENCES", "User saved preferences", user["id"])
...
def get_saved_searches():
    ...
    conn = get_db_connection()
    rows = conn.execute(
        "SELECT id, search_name, filters_json, created_at FROM saved_searches WHERE user_id = ? ORDER BY id DESC",
        (user["id"],),
    ).fetchall()
    conn.close()
...
def save_search():
    ...
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO saved_searches (user_id, search_name, filters_json) VALUES (?, ?, ?)",
...
def delete_saved_search(search_id: int):
    user = current_user()
    conn = get_db_connection()
    conn.execute("DELETE FROM saved_searches WHERE id = ? AND user_id = ?", (search_id, user["id"]))
    conn.commit()
    conn.close()
...
def admin_users():
    conn = get_db_connection()
    rows = conn.execute("SELECT id, name, email, role, is_active, created_at FROM users ORDER BY id DESC").fetchall()
    conn.close()
...
def admin_logs():
    conn = get_db_connection()
    rows = conn.execute("""
        SELECT system_logs.id, system_logs.action, system_logs.details, system_logs.created_at,
...
def admin_retrain_model():
    ...
    return jsonify({"message": "Retrain request logged. Run python train_model.py from terminal to retrain safely."})
